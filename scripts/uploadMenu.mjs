...
function getImageItemName(filename) {
  return filename
...
function findImagePath(menuPath, category, itemName) {
...
async function uploadImageToSupabase(imagePath, itemName, category) {
  try {
...
async function uploadTerrazaMenu(menuPath) {
  try {
...
// Run the upload
const menuPath = 'C:\\Users\\Admin\\Downloads\\extracted_menu\\Terraza Navarro MENU';

console.log('🚀 Natalna\'s Restaurant - Menu Upload Tool\n');
console.log(`📁 Menu Path: ${menuPath}\n`);

uploadTerrazaMenu(menuPath)
  .then(result => {
    if (result.success) {
